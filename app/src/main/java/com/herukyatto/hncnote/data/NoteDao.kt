...
interface NoteDao {
    ...
    @Query("SELECT * FROM notes_table WHERE is_in_trash = 0 AND folder_id = :folderId ORDER BY is_favorite DESC, last_modified DESC")
    fun getNotesSortedByDateDesc(folderId: Int): Flow<List<Note>>

    @Query("SELECT * FROM notes_table WHERE is_in_trash = 0 AND folder_id = :folderId ORDER BY is_favorite DESC, last_modified ASC")
